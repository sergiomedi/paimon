...
param environmentName = readEnvironmentVariable('AZURE_PAIMON_ENV', 'dev')
...
param location = readEnvironmentVariable('AZURE_PAIMON_LOCATION', 'swedencentral')

// az ad signed-in-user show --query id -o tsv
param operatorPrincipalId = readEnvironmentVariable('AZURE_PAIMON_OPERATOR_ID', '')
...
param administratorPrincipalName = readEnvironmentVariable('AZURE_PAIMON_OPERATOR_NAME', '')

// The image to run, registry included. Set by scripts/azure/deploy.sh, which
// resolves it from the registry when AZURE_PAIMON_API_IMAGE is not set and refuses to
...
param apiImage = readEnvironmentVariable('AZURE_PAIMON_API_IMAGE', 'none')
...
param deployApi = readEnvironmentVariable('AZURE_PAIMON_DEPLOY_API', 'true') == 'true'
...
param apiAudience = readEnvironmentVariable('AZURE_PAIMON_API_AUDIENCE', '')
...
param modelPrices = json(readEnvironmentVariable('AZURE_PAIMON_MODEL_PRICES', '{}'))
